...
const std::vector<int> DNA_OVERRIDE = {
    ...
    // 1000p 100g 20 macas 915, -340, 207, 635, 838, -124, -718, 53, -852, 233, 499, -483, -946, -517, -377, 502, -469, -371, -552, -226, -781, 41, -746, 608, -544, -46, -559, -288, 895, -10, 845, 425, -503, -464, 477, -825, 836, -936, 115, -13, 773, 265
    // 5000p 500g 30 macas 981, 499, -481, -89, 874, -754, 216, 123, 936, -188, 129, -652, 855, -103, 155, 269, -986, -435, -721, 648, -924, 834, 693, -941, 252, 547, -266, 747, 177, -128, 89, -149, 860, 945, -373, -880, -974, -519, 160, 242, 63, -196
    // 10000p 100g 981, 499, -481, -89, 874, -754, 216, 123, 936, -188, 129, -652, 855, -103, 155, 269, -986, -435, -721, 648, -924, 834, 693, -941, 252, 547, -266, 747, 177, -128, 89, -149, 860, 945, -373, -880, -974, -519, 160, 242, 63, -196
    // 20000p 200g
    -763, 765, 242, 619, 811, 964, 457, -119, -225, -168, 463, -476, -741, 318, -504, -533, -183, 141, 152, 698, -999, 105, 199, -168, -30, -684, 427, -855, -734, 252, -457, 910, -624, -523, -264, -718, -282, 435, -916, -682, 186, -1000
};
...
double FitnessFunction(const std::vector<int>& chromosome) {
    ...
    for (int gene : chromosome)
        dna.push_back(gene / 1000.0f); // Normaliza genes para [-1, 1]
    SnakeGame env;
    NeuralNet net(dna);

    int steps_taken = 0;
    while (!env.IsGameOver()) {
        auto sensors = env.GetSensors();      // Lê sensores do ambiente
        int action = net.Decide(sensors);     // Rede neural decide ação
        env.Step(action);                     // Executa ação
        steps_taken++;
    }
    // Fitness: recompensa maçãs (peso alto) e penaliza passos (peso baixo)
